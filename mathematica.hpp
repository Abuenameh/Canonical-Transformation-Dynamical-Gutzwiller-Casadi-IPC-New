...
ostream& operator<<(ostream& out, const mathematica<interprocess::basic_string<T, Traits, Alloc>>& m) {
    out << "\"" << m.v << "\"";
    return out;
}
...
template<typename T, typename Alloc>
ostream& operator<<(ostream& out, const mathematica<interprocess::vector<T, Alloc> >& m) {
    interprocess::vector<T, Alloc>& d = m.v;
    out << "{";
    if (d.size() > 0) {
        out << mathematica<T > (d[0]);
    }
